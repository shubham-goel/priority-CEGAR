...
\section{Computational complexity}
	...
	% Each clause message's path intersects exactly one of the message paths of every variable it contains, depending on whether the variable appears as a positive literal or a negative one. Clause message $m_C$'s path intersects $p_x$ if $x$ appears as a positive literal in $C$, similarly for $p_{\neg x}$.

	% If $\sigma \models \psi$ corresponds to the crash sequence $\zeta$, then $\zeta$ leads to a {bad} outcome where no {\it Clause} message arrives at its destination on time.

	% Unlike the {\it Variable} messages, each clause message $m_C$ has a fixed path which it always travels on. This intersects with one of the message paths of every variable it contains, depending on whether the variable appears as a positive literal or a negative one. Clause message $m_C$'s path intersects $p_x$ if $x$ appears as a positive literal in $C$, similarly for $p_{\neg x}$. All {\it Variable} messages are set to have a higher priority than all {\it Clause} messages. This ensures that whenever a {\it Variable} and a {\it Clause} message appear in the same vertex queue, the {\it Clause} message gets delayed. All messages are carefully synchronized using redundant edges such that a {\it Variable} message $m_x$ travelling on $p_{\neg x}$ always delays an intersecting and previously undelayed {\it Clause} message by making it wait at the vertex at which the intersection occurs. Further, the total length of the paths of each {\it Clause} messages equals the global timeout $t$. Hence, delayed {\it Clause} messages do not reach on time. Also note that only `satisfied' {\it Clause} messages are delayed and path lengths of the {\it Variable} messages are made arbitrarily long to prevent them from ever reaching their destination. It is evident by now, how and why the one-one correspondence works.
	...
	\subsection{Counting to Scoring}
		...
		\end{itemize}
		% \noindent These nodes and edges $2$ ``message'' paths for each variable:  $p_v = {p_v}_0 \ {p_v}_1 \ {p_v}_2 \ \ldots \ {p_v}_t$ and ${p_{\neg v} = p_v}_0 \ {p_{\neg v}}_1 \ {p_{\neg v}}_2 \ \ldots \ {p_{\neg v}}_{t-1} \ {p_v}_t$. Here, $t$ is the global timeout, the value of which we will decide later. Note that the paths corresponding to different messages are disjoint. For each clause $C$, we shall construct a new path $p_C$ of of length $t$ such that:

		% $$\forall x \in \bigcup_{v\in \chi} \set{v,\neg v}, x \models C \Rightarrow p_C \cap p_x = ({p_x}_i,{p_x}_{i+1})$$

		% If $x \models C$, $p_C$ and $p_x$ share exactly one edge $({p_x}_i,{p_x}_{i+1})$. We will additionally ensure that that $({p_x}_i,{p_x}_{i+1})$ be the ${i+1}^{th}$ edge in $p_C$. $p_C$ shall not share any edges/nodes with any clause path or message path that corresponds to a literal that doesn't lies in C.
		...
		\noindent {\em Crash Probabilities}:
		\[
		    p_{crash}(e) =
		\begin{cases}
		...
		\end{cases}
		\]
		\noindent {\em Edge \& Message Priorities}:

		Let $\chi(k)$ denote the $k^{th}$ variable in $\chi$, and $\C(k)$ denote the $k^{th}$ clause in $\C$.
		% $$\forall k : 1 \leq k < \Vert\C\Vert : m_{\C(k)} > m_{\C(k+1)}$$
		% $$\forall k : 1 \leq k < \Vert\chi\Vert : m_{\chi(k)} < m_{\chi(k+1)}$$
		...
		$$\sigma(v) = \ff \iff ({p_v}_0,{p_v}_1) \text{ crashes at time }0 $$ %\iff m_v \text{ travels on } p_{\neg v}$$
